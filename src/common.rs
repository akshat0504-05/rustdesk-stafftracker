...
pub const PORTABLE_APPNAME_RUNTIME_ENV_KEY: &str = "STRemote_APPNAME";
...
// Because the url is always `https://api.STRemote.com/version/latest`.
#[tokio::main(flavor = "current_thread")]
pub async fn do_check_software_update() -> hbb_common::ResultType<()> {
    let (request, url) =
        hbb_common::version_check_request(hbb_common::VER_TYPE_STRemote_CLIENT.to_string());
...
#[inline]
pub fn is_STRemote() -> bool {
    hbb_common::config::APP_NAME.read().unwrap().eq("STRemote")
}
...
pub fn is_public(url: &str) -> bool {
    ...
    url.contains("STRemote.com/") || url.ends_with("STRemote.com")
}
...
pub fn check_process(arg: &str, mut same_uid: bool) -> bool {
    ...
    for (_, p) in sys.processes().iter() {
        ...
        // on mac, p.cmd() get "/Applications/STRemote.app/Contents/MacOS/STRemote", "XPC_SERVICE_NAME=com.carriez.STRemote_server"
...
pub type STRemoteInterval = ThrottledInterval;

#[inline]
pub fn STRemote_interval(i: Interval) -> ThrottledInterval {
...
mod tests {
    ...
    #[tokio::test]
    async fn test_STRemote_interval() {
        ...
        for maker in base_intervals.into_iter() {
            ...
            let mut timer = STRemote_interval(maker());
    ...
    #[allow(non_snake_case)]
    #[tokio::test]
    async fn test_STRemote_interval_sleep() {
        let base_intervals = [interval_maker, interval_at_maker];
