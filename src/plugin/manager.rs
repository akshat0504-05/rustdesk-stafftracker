...
fn get_plugin_source_list() -> Vec<PluginSource> {
    ...
    // vec![PluginSource {
    //     name: "STRemote".to_string(),
    //     url: "https://raw.githubusercontent.com/fufesou/STRemote-plugins/main".to_string(),
...
fn elevate_install(
    ...
    crate::platform::elevate(args, "STRemote wants to install then plugin")
}
...
fn elevate_uninstall(plugin_id: &str) -> ResultType<bool> {
    crate::platform::elevate(
        ...
        "STRemote wants to uninstall the plugin",
    )
}
