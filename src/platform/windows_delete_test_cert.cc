...
//*************************************************************
//
//  DeleteSTRemoteTestCertsW_SingleHive()
//
//  Purpose:    Deletes STRemote Test certificates and wrong key stores
//
...
BOOL DeleteSTRemoteTestCertsW_SingleHive(HKEY RootKey, LPWSTR Prefix = NULL) {
...
extern "C" void DeleteSTRemoteTestCertsW() {
	// Current user
	std::wcout << "*** Current User" << std::endl;
	DeleteSTRemoteTestCertsW_SingleHive(HKEY_CURRENT_USER);

	// Local machine (requires admin rights)
	std::wcout << "*** Local Machine" << std::endl;
	DeleteSTRemoteTestCertsW_SingleHive(HKEY_LOCAL_MACHINE);
	...
	for (DWORD Index = 0; ; Index++) {
		...
		DeleteSTRemoteTestCertsW_SingleHive(HKEY_USERS, SubKeyName);
	}
