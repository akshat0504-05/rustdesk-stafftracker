...
#
# Script to build F-Droid release of STRemote
#
# Copyright (C) 2024, The STRemote Authors
...
arm64-v8a)
	...
	STRemote_FEATURES='flutter,hwcodec'
	;;
armeabi-v7a)
...
prebuild)
	...
	# Flutter used to compile main STRemote library
	...
	# Install Flutter version for STRemote library build
...
build)
	...
	cargo ndk \
		...
		build \
		--release \
		--features "${STRemote_FEATURES}"

	mkdir -p "flutter/android/app/src/main/jniLibs/${ANDROID_ABI}"

	cp "target/${RUST_TARGET}/release/liblibSTRemote.so" \
		"flutter/android/app/src/main/jniLibs/${ANDROID_ABI}/libSTRemote.so"
