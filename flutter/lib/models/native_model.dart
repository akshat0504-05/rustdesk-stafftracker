...
class PlatformFFI {
  ...
  STRemoteImpl get ffiBind => _ffiBind;
  ...
  Future<void> init(String appType) async {
    ...
    final dylib = isAndroid
        ? DynamicLibrary.open('libSTRemote.so')
        : isLinux
            ? DynamicLibrary.open('libSTRemote.so')
            : isWindows
                ? DynamicLibrary.open('libSTRemote.dll')
                :
                ...
                //
                // isMacOS? DynamicLibrary.open("liblibSTRemote.dylib") :
  ...
  void _startListenEvent(STRemoteImpl STRemoteImpl) {
    final appType =
        _appType == kAppTypeDesktopRemote ? '$_appType,$kWindowId' : _appType;
    var sink = STRemoteImpl.startGlobalEventStream(appType: appType);
    sink.listen((message) {
      () async {
        try {
