...
#define STRemote_LIB_PATH "libSTRemote.so"
typedef bool (*STRemoteCoreMain)();
bool gIsConnectionManager = false;

void print_help_install_pkg(const char* so);

bool flutter_STRemote_core_main() {
   void* libSTRemote = dlopen(STRemote_LIB_PATH, RTLD_LAZY);
   if (!libSTRemote) {
      fprintf(stderr,"Failed to load \"libSTRemote.so\"\n");
      char* error;
   ...
   auto core_main = (STRemoteCoreMain) dlsym(libSTRemote,"STRemote_core_main");
   char* error;
   if ((error = dlerror()) != nullptr) {
       fprintf(stderr, "Program entry \"STRemote_core_main\" is not found: %s\n", error);
       return false;
   }
   return core_main();
}
